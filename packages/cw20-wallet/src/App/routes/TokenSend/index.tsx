...
function TokenSend(): JSX.Element {
  ...
  const sendTokensAction = (values) => {
    setLoading(true);
    const { address: recipientAddress, amount: transferAmount } = values;
      ...
      if (allowingAddress) {
        cw20Contract.transferFrom(allowingAddress, recipientAddress, transferAmount).then((txHash) => {
          ...
          history.push({
            ...
            state: {
              success: true,
              message: `${tokenAmount} ${tokenName} succesfully sent to ${recipientAddress} with allowance from ${allowingAddress}`,
        ...
        cw20Contract.transfer(recipientAddress, transferAmount).then((txHash) => {
          ...
          history.push({
            ...
            state: {
              success: true,
              message: `${tokenAmount} ${tokenName} succesfully sent to ${recipientAddress}`,
              customButtonText: "Tokens",
            } as OperationResultState,
          });
        });
